...
class Transformer(TransformerBaseClass):
    def __init__(self, num_classes, learning_rate, batch_size, decay_steps, decay_rate, sequence_length,
        ...
        super(Transformer, self).__init__(d_model, d_k, d_v, sequence_length, h, batch_size,
                                          num_layer=num_layer)  # init some fields by using parent class.
    ...
    def loss(self, l2_lambda=0.0001):  # 0.001
        with tf.name_scope("loss"):
            ...
            losses = tf.nn.sparse_softmax_cross_entropy_with_logits(labels=self.input_y_label,
            ...
            l2_losses = tf.add_n(
                [tf.nn.l2_loss(v) for v in tf.trainable_variables() if ('bias' not in v.name) and ('alpha' not in v.name)]) * l2_lambda
            loss = loss + l2_losses
        return loss
...
def test_training():
    ...
    with tf.Session() as sess:
        ...
        for i in range(15000):
            ...
            print(i, "loss:", loss, "acc:", acc, "label_list_original as input x:", label_list_original, ";input_y_label:", input_y_label,
                  "prediction:", predict)
            if i % 1500 == 0:
...
def test_predict():
    ...
    with tf.Session() as sess:
        ...
        for i in range(1500):
            ...
            print(i, "label_list_original as input x:", label_list_original, "prediction:", predict, ";label:",
                  input_y_label)  # "acc:", acc, "loss:", loss ";input_y_label:", input_y_label
